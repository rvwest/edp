...
<form action="<?php echo esc_url($action); ?>" method="post" id="stripe-checkout-form" class="job-manager-form"
	data-secret="<?php echo esc_attr($intent_client_secret); ?>">
	...
	<div class="job_listing_stripe_checkout_form ">
		<fieldset class="fieldset-stripe_payment fieldset-type-text">
			<div class="job-details-block">
				...
				<div>
					<span class="item-cost"><?php echo esc_html($item_cost); ?></span>
				</div>
			</div>
		</fieldset>
		...
		<div class="option-cols">
			<div class="option-col1">
				...
				<h2>Pay by card <a href="https://stripe.com/gb"><img class="stripe-badge" alt="Powered by Stripe"
							src="/wp-content/themes/edpsy/images/powered_by_stripe.svg" /></a></h2>
				...
				<fieldset class="fieldset-name fieldset-type-text">
					...
					<div class="field">
						<div class="stripe-name-field">
							<input type="text" class="input-text" name="cardholder-name" id="stripe-cardholder-name"
								required />
						</div>
					</div>
				</fieldset>

				<p><i class="fas fa-badge-percent" style="color:#bbb"></i> <a
						href="https://buy.stripe.com/00g6qAePPcvA4fKfZa?prefilled_email=<?php echo esc_html(wp_get_current_user()->user_email); ?>&client_reference_id=<?php echo esc_attr($job_id); ?>">Use
						a promotion code</a></p>
				...
				<input type="submit" name="submit_payment" class="button"
					value="Pay <?php echo esc_html($item_cost); ?>" />
				<span class="spinner"
			...
			<div class="option-col2">
				...
				<p class="option-block-top-text"><i class="fas fa-check"></i> We've saved your listing.</p>

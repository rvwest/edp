...
<form action="<?php echo esc_url($action); ?>" method="post" id="submit-job-form" class="job-manager-form"
	...
	<?php if (job_manager_user_can_post_job() || job_manager_user_can_edit_job($job_id)): ?>
		...
		if (isset($resume_edit) && $resume_edit) {
			printf('<p><strong>' . esc_html__("You about to edit an existing job. %s", 'wp-job-manager') . '</strong></p>', '<a href="?new=1&key=' . esc_attr($resume_edit) . '">' . esc_html__('Create a new job', 'wp-job-manager') . '</a>');
		}
		?>
